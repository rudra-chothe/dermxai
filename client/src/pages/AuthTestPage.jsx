...
const AuthTestPage = () => {
  ...
  const testGetToken = async () => {
    try {
      ...
      if (token) {
        ...
        addTestResult("Get ID Token", true, "Successfully retrieved Firebase ID token");
      } else {
  ...
  const testVerifyToken = async () => {
    try {
      ...
      addTestResult("Verify Token", true, "Token verified successfully", response);
    } catch (error) {
  ...
  const testGetProfile = async () => {
    try {
      ...
      addTestResult("Get Profile", true, "Profile retrieved successfully", response);
    } catch (error) {
  ...
  const testProtectedRoute = async () => {
    try {
      ...
      addTestResult("Protected Route", true, "Accessed protected route successfully", response);
    } catch (error) {
  ...
  const testPublicRoute = async () => {
    try {
      ...
      addTestResult("Public Route", true, "Accessed public route successfully", response);
    } catch (error) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-sm">
                    <div>
                      ...
                      <span className="ml-2 font-mono text-xs">{currentUser.uid}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Badge 
                        variant={currentUser.emailVerified ? "success" : "destructive"}
                        className={`ml-2 ${currentUser.emailVerified ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                      >
                  ...
                  {tokenInfo && (
                    <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                      <h4 className="font-medium text-sm mb-2">Current Token</h4>
                      <p className="text-xs font-mono break-all">{tokenInfo.token}</p>
                      <p className="text-xs text-gray-600 mt-1">Length: {tokenInfo.length} characters</p>
                    </div>
                  )}
                </div>
              ) : (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Run authentication and API tests</CardDescription>
            </CardHeader>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <span className="ml-2">{import.meta.env.VITE_API_URL || 'http://localhost:5000'}</span>
              </div>
              <div>
                <span className="font-medium">Firebase Project:</span>
                <span className="ml-2">{import.meta.env.VITE_FIREBASE_PROJECT_ID}</span>
              </div>
              <div>
                <span className="font-medium">Auth Domain:</span>
                <span className="ml-2">{import.meta.env.VITE_FIREBASE_AUTH_DOMAIN}</span>
              </div>
              <div>
                <span className="font-medium">Use Firebase Auth:</span>
                <span className="ml-2">{import.meta.env.VITE_USE_FIREBASE_AUTH || 'true'}</span>
              </div>
            </div>
          </CardContent>
        </Card>
